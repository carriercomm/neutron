...
class QoSServerRpcMixin(qos_db.QoSDbMixin):
    ...
    def process_port_qos_update(self, context, id, port):
        if ext_qos.QOS not in port['port']:
            return
        mapping = self.get_mapping_for_port(context, id)
        qos_id = super(QoSServerRpcMixin, self).process_port_qos_update(context, id, port)
        if qos_id is None:
            self._process_delete_qos_for_port(context, mapping[0].qos_id, id)
        elif mapping == [] and qos_id is not None:
            self._process_create_qos_for_port(context, qos_id, id)
        elif mapping != [] and qos_id != mapping[0].qos_id:
            self._process_update_mapping_for_port(context, mapping[0])
