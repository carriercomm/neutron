...
class OVSQoSAgent(qos_rpc.QoSAgentRpcMixin):
    ...
    def check_reapply_qos_policy(self, context, port):
        qos_id = self.plugin_rpc.get_qos_by_port(context, port.vif_id)
        if qos_id != []:
            self.port_qos_updated(context, qos_id, port.vif_id)
    ...
    def port_bound(self, port, net_uuid,
        ...
        if cur_tag != str(lvm.vlan):
            ...
            if port.ofport != -1:
                ...
                self.qos_agent.check_reapply_qos_policy(self.context, port)
