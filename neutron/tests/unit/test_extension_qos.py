...
class TestQoS(QoSDBTestCase):
        ...
        with self.network() as n:
            with self.subnet(n):
                with self.qos(self.default_qos) as qos:
                    ...
                    data = {'port': {ext_qos.QOS: qos['qos']['id']}}
    ...
    def test_update_port_unrelated_attribute(self):
        with self.network() as n:
            with self.subnet(n):
                with self.qos(self.default_qos) as qos:
                    res = self._create_port(self.fmt, n['network']['id'])
                    ...
                    data = {'port': {'name': 'new-name'}}

                    req = self.new_update_request('ports', data,
                                                  port['port']['id'])
                    res = self.deserialize(self.fmt,
                                           req.get_response(self.api))
                    self.assertIn(ext_qos.QOS, res['port'])
                    self._delete('ports', res['port']['id'])
